...
**② SELF-AUDIT + SELF-IMPROVING** — `self-audit-skill MICRO_RESPONSE_MODE` fires before the footer using a **SEPARATE GRADER PASS** (fresh context, different prompt from the generator — same context self-grading is gameable). **SHIP GATE: score must reach ≥9/10** before the footer prints. If score <9: iterate (max 3 iterations). **STUCK DETECTOR:** if score improvement between iterations is <0.3, STOP iterating and escalate to Bennett with "reached X/10, here is what is missing" — do NOT infinite-loop. Score **≤8.5** triggers `Skill("council-skill")` Permanent Fix Approval Mode; **≤7** triggers escalation. Footer MUST carry `self-audit: pass | corrected | escalated | partial:<blocker>`. **MECHANICAL ENFORCEMENT (v9.0/v9.2):** "corrected" status requires proof of a council-skill tool call AND a skill/file edit receipt in this response. "corrected" without those = false-done on the self-audit itself. [PERMANENT FIX 2026-06-03: ship gate raised from 7/10 to 9/10 per Bennett directive; iteration cap + stuck-detector added per council blocking condition 1; grader separation added per council blocking condition 2.]
