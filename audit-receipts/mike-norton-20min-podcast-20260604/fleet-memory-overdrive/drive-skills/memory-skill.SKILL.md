...
description: "Update all persistent storage when Bennett says update memory, memory protocol, update everything, sync memory, save this, audit memory, or use the memory skill. Handles cross-machine memory reads via SSH to Mack. v9.6 adds No-Early-Stop fleet behavior: Yellow/partial/AI-open becomes an execution queue, not a final state, whenever a safe reversible tool path remains. MANDATORY TRIGGERS: update memory, memory protocol, update everything, sync memory, save this, audit memory, use the memory skill."
credential_vault_page_id: 341cf5514fd381fe993de8add7eb265e
deps:
  - name: closeout-skill
    fileId: 1udXAN7CVGspKw3OIEfSdz9ZZpT_6BuGy
...
# Memory Protocol Skill v9.6 (2026-06-04)
# v9.6 patch (2026-06-04): Added NO-EARLY-STOP fleet behavior. Yellow/partial/AI-open is not a closeout state when a safe, reversible, no-send/no-delete/no-secret tool path remains; the agent must execute up to 3 safe blocker-clearing actions, then rerun gatekeeper/manifest/fetchback. Only true human/protected gates or platform-unavailable blockers may remain open, and they require proof.
...
## No-Early-Stop Fleet Behavior (v9.6 — HARD RULE)

When a memory/governance run returns Yellow, partial, AI OPEN, or a blocker:

1. Treat the status as an execution queue, not a final answer.
2. Ask: is there a safe next action that is reversible, no-send, no-delete, no-money, no-legal-send, and does not expose secrets?
3. If yes, execute that next action now using Drive/Notion/tooling, then fetch back/read back proof and rerun the relevant gate. Repeat for up to 3 safe attempts before requiring a bounded continuation receipt.
4. If no, record the exact human/protected/platform blocker, the failed probes, and the next owner.
5. Do not close, recap as complete, claim memory saved, claim Diamond, or call the project finished while AI-owned executable work remains.
6. A local receipt alone is not a fleet behavior change; the canonical Drive/Notion source must be updated and fetched back.

This rule exists because a prior Chad/Codex run stopped at Yellow after finding a stale `human-gate-skill` manifest dependency. Correct behavior was to patch the Drive-canonical `memory-sync-skill`, fetch it back, and rerun L8 to Green.
...
No-early-stop addendum (v9.6): If this workflow produces Yellow/partial status and any safe AI-owned tool action remains, the agent must execute that next action before finalizing. Yellow can be final only when the remaining item is a true human/protected/platform blocker with probe proof.
