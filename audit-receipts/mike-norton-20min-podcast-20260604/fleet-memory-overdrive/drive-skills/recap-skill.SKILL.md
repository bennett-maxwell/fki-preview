## v10.1 Fresh Visible Canonical File-ID Repair

Drive folder-first discovery must use this fresh visible `SKILL.md` file `1v-ySRRXR-B8Wqf4BGu-hIMFfUqMj2tQe`. The previous file ID `1e9nw03yKQ9tjD4k3zEBAhQsEnH0qZWe6` is in Trash: it may be useful as historical proof, but it is not folder-discoverable and must not be treated as the active canonical patch target. Future Drive-first agents should patch this file ID in place and fetch back this file after replacement.

---
name: recap-skill
version: 10.1
drive_file_id: 1v-ySRRXR-B8Wqf4BGu-hIMFfUqMj2tQe
last_updated: 2026-06-04
v10_1_fresh_visible_canonical: "2026-06-04 — FRESH VISIBLE CANONICAL: created a new untrashed folder-visible SKILL.md 1v-ySRRXR-B8Wqf4BGu-hIMFfUqMj2tQe because prior canonical recap files were in Trash and folder-first agents could not discover them. This file is now the active Drive-folder canonical patch target."
v10_0_visible_canonical_repair: "2026-06-04 — VISIBLE CANONICAL FILE-ID REPAIR: Drive folder discovery now returns this visible SKILL.md file 1r3zNdQmcOn1w-S4DFDtyNHmsX39mcmez. Retired file 1e9nw03yKQ9tjD4k3zEBAhQsEnH0qZWe6 is trashed and must not be used for folder-first loading. Updated frontmatter to the visible canonical file ID so Drive-first agents patch the folder-visible SKILL.md in place."
v9_9_number_namespace_fix: "2026-06-04 — Bennett correction: avoid two visible bare number threes. The Q&A roll-up header must use Q1., Q2., etc. or short bullets, never bare 1./2./3./4./5. lines. The final five next options are the only bare numbered lines; option 3 remains the recommended format `3. ⭐ ...`. Codex fallback checkers must fail drafts that contain any extra bare 1.-5. lines outside the final option block."
v9_8_ai_open_no_stop_gate: "2026-06-04 — AI-OPEN NO-STOP GATE: recap cannot be used as a stopping wrapper while AI-owned tasks remain. Before final, parse AI OPEN. If any item is AI-executable and not blocked by human/legal/finance/protected-tool/platform limit, execute it before sending. If platform/tool limit prevents execution, create or reference a bounded continuation receipt (thread heartbeat, Notion next action, or Drive/Notion receipt) and label recap partial: ai_open_continuation_required. Final must then run self-audit-skill on the exact draft and show whether AI OPEN is zero, human-blocked, or continuation-backed."
v9_6_session_memory_gate: "2026-06-04 — Bennett permanent fix: recap alone is not enough. A response may not close unless a session-scoped memory-skill receipt exists. The Claude stop hook now blocks on BOTH recap-fire and memory-skill receipt. No-op memory scans still require a receipt; durable memory claims still require canonical Drive/Notion proof."
v9_4_codex_memory_canon_fix: "2026-06-03 — Bennett directive: Codex/ChatGPT local ad_hoc memory notes, local workspace recap receipts, and local receipt paths no longer count as saved-memory proof. They may support reconciliation only. Canonical memory proof must point to Drive/Notion canon or a memory-skill receipt tied to that canonical write."
v9_5_durable_claim_proof_gate: "2026-06-04 — Bennett correction: permanent/fleet-wide/canonical/shared-memory claims require durable proof for the exact changed source. A recap-skill proof line proves only recap source loading; it does not prove a permanent change. Durable proof means Drive/Notion canonical write plus fetchback/readback, gog replaced:true plus SHA/size, or memory-skill receipt tied to canonical Drive/Notion write. Local checker files, local receipts, local ad_hoc notes, and Notion project-state updates alone must be labeled partial/yellow, never Green/perfect/permanent."
v9_2_cloud_agent_fix: "2026-06-03 — PERMANENT FIX (council 3.13/5, Skorl audit): Extended identity gate in End-of-Thread Council Loop to include cloud/Hyperagent agents (Skorl). Prior gate only handled temp→mack, openclaw→ivan, else exit 0 — cloud surface had 0% autonomous loops. Cloud agents now use Drive+Notion as state layer. Also documents that council loop uses 5 options × 4 rounds = 20 total (NOT 50+50 — Bennett's '50 ideas expand + 50 fail' description is unimplemented; needs explicit council option to add if desired). Auditor confirmed system at 48% of 3-pillar potential fleet-wide."
lss_score: 96
...
v9_2_diamond_hard_gate: "v9.2 (2026-06-03): FRC-5 upgraded to HARD GATE — Diamond T7 auto-fires on any gog replaced:true or Write-success result. Fixes the 'Diamond is manual-only' audit gap. Before the footer prints, agent scans tool call outputs for replaced:true or Write success+path; if any Drive skill patch / file write detected and no diamond-skill T7 receipt exists → BLOCK footer → invoke diamond-skill T7 now → continue only after PASS. T7 FAIL → mark 🟡 + surface diamond_auto_fail in footer. Exception: pure Slack/research outputs with no persistent file. BLOCK 0 preamble updated to note FRC-5 is now a HARD GATE."
v9_1_execute_dispatch: "2026-06-02 — EXECUTE-DISPATCH ENFORCEMENT: after council-skill returns EXECUTE-rated items, agent MUST dispatch a tool call for each before advancing the round. v9.1 complements v9.0 (council CALLED) by enforcing EXECUTE items are DISPATCHED. Fix: sprint row recap-skill 4-round auto-execute not firing."
...
## v9.8 AI OPEN NO-STOP CLOSEOUT GATE

Recap is not a permission to stop with agent-owned work still open. Before any non-lite final response:

1. Build an `ai_open_items[]` list from the draft, Notion row, and current tool results.
2. For each item, classify `owner = ai | human | protected | platform_limit`.
3. If any item is `owner=ai`, the final answer is BLOCKED. Execute it now, then rebuild the draft.
4. If the item cannot be executed because the current platform cannot continue after final, create a bounded continuation receipt first (thread heartbeat, Notion next action with timestamp, or Drive/Notion continuation receipt). Label `recap partial: ai_open_continuation_required`; do not call the work done.
5. Run self-audit-skill on the exact final draft after the AI OPEN classification. Self-audit must fail if the draft says “AI OPEN” with executable items and no continuation receipt.
6. The footer may show AI OPEN only when every item is human/protected/platform-limited and has a proof-backed blocker or continuation reference.

This is a hard closeout gate for correction, permanent-fix, handoff, recap, full-autonomy, and self-improvement prompts.


# Recap Skill v9.2 (runtime). Version history → recap-skill-CHANGELOG.md

## ⛔ BLOCK 0 — MANDATORY EXECUTION CONTRACT

**FRC-5 is now a HARD GATE — Diamond T7 auto-fires on any Drive write (v9.2, 2026-06-03, Bennett directive). Agents do NOT need to remember to invoke diamond-skill after a write; it is mechanically enforced. See FRC-5 below.**

**Run `Skill("memory-skill")` before the final answer, then call `Skill("recap-skill")` at the END OF EVERY RESPONSE.** The Stop hook (`auto-closeout-gate.sh` v2.2 + `inject-recap-reminder.sh` v1.4) now mechanically blocks a response that is missing either recap-fire OR a session-scoped memory-skill receipt; after 2 blocks → escape + log to `recap-enforcement.log` for Angie.

**Codex/ChatGPT fallback:** if this surface cannot call `Skill("recap-skill")` or use the Claude Stop hook, the agent must manually execute the recap contract before the final response. Required proof: load this canonical Drive `SKILL.md` and record `drive_file_id` + `version`; write a workspace recap receipt with task, artifacts, proof, AI open, human open, self-audit, and memory/Notion status; append `~/.openclaw/state/recap-fire-ledger.jsonl`; label any missing Drive/Notion side effect as `recap partial:<blocker>`; include a final `recap-skill:` closeout line naming the receipt path. That workspace recap receipt is an operational receipt only — never memory-proof by itself. A Codex response without that fallback receipt is incomplete.
...
**THREE MANDATORY JOBS EVERY NON-LITE RESPONSE (v9.7, 2026-06-04, Bennett directive — ALL THREE, ALWAYS):**

**① FLEET MEMORY SAVE** — After every single response, save session memory for the ENTIRE fleet: (a) append memory-skill receipt to `~/.openclaw/state/memory-skill-receipts.jsonl`; (b) push any changed memory files to Drive canonical via gog; (c) update Notion Live Thread row; (d) write Sprint Board task-state update if a task changed status. This is NOT optional. A response that closes without fleet memory saved = INCOMPLETE regardless of all other scores.

**② SELF-AUDIT + COUNCIL LOOP UNTIL 95%** — `self-audit-skill MICRO_RESPONSE_MODE` fires before the footer using a **SEPARATE GRADER PASS** (fresh context, different prompt from the generator — same context self-grading is gameable). **SHIP GATE: score must reach ≥9.5/10 (95%)** before the footer prints. If score <9.5: (a) run council-skill on EVERY gap found; (b) implement ALL council fixes; (c) re-run self-audit; (d) repeat until 95%+ or stuck-detector triggers. **STUCK DETECTOR:** if score improvement between iterations is <0.3, STOP iterating and escalate to Bennett with "reached X/10, here is what is missing" — do NOT infinite-loop. Score **≤9** triggers `Skill("council-skill")` Permanent Fix Approval Mode; **≤7** triggers escalation. Footer MUST carry `self-audit: pass | corrected | escalated | partial:<blocker>`. **MECHANICAL ENFORCEMENT (v9.0/v9.2):** "corrected" status requires proof of a council-skill tool call AND a skill/file edit receipt in this response. [PERMANENT FIX 2026-06-04: ship gate raised to 9.5/10 (95%); council-loop-until-95% mandatory.]

**③ AUTONOMOUS LOOPS** — Multiple rounds of full autonomous loops at END-OF-THREAD ONLY (see firing rule). Each round: (a) council-skill fires; (b) EXECUTE-rated items are dispatched; (c) self-audit re-runs on any output; (d) score must be ≥95% before round closes. ≤4 items/round; 5-min cap/round; default rounds unless Bennett specifies more. If any round closes below 95%: run council again, fix, repeat — do NOT ship a sub-95% round as complete.
...
- **FRC-5 Diamond HARD GATE (v9.2 — 2026-06-03, Bennett directive):** Before the footer prints, scan this turn's tool call outputs for `replaced: true` (gog upload) OR `Write success` + path. If ANY Drive skill patch / file write is detected AND no diamond-skill T7 receipt exists in this turn's outputs:
  → BLOCK footer
  → invoke diamond-skill T7 (Persisted-Write Verification) on the patched file NOW
  → only continue to footer after T7 receipt returns PASS
  → if T7 returns FAIL: mark response 🟡, surface "diamond_auto_fail: [file]" in footer
  This gate fires automatically — agents do NOT need to remember to invoke diamond-skill after a write. Diamond is now mechanically enforced on every Drive write.
  Exception: pure Slack/research outputs with no persistent file → skip.
...
**Number namespace rule (v9.9):** To avoid Bennett seeing duplicate bare numbers, Q&A lines use `Q1.`, `Q2.`, etc. or bullets. Do not use bare `1.`, `2.`, `3.`, `4.`, or `5.` in this header. The final next-action options are the only bare numbered lines in the response, and the recommendation stays `3. ⭐ ...` when slot 3 is recommended.
```
...
## End-of-Thread Council Loop (Mack + Ivan + Cloud — never Leo, never intermediate responses)

**Identity gate (HARD):** `whoami` primary — `temp`→mack, `openclaw`→ivan, else hostname fallback (`macbook`→mack, `imac`→ivan), else check `HYPERAGENT_SESSION` or `SKORL_SESSION` env vars → `cloud`; else leo → `exit 0` (loop never fires on Leo). **Cloud agents (Skorl/Hyperagent) run the loop using Drive + Notion as state layer** instead of local files: receipts written to Drive folder `1qdUEbUb_BpkVBt_YCX686nnu-O7lW0pY`, memory state to Notion Live Thread row, option_hashes deduped in-memory per session. Cloud loop fires on same END-OF-THREAD conditions as Mack/Ivan. [PERMANENT FIX 2026-06-03 council: cloud-agent identity gap caused 0% autonomous loops on Hyperagent surface.]
...
**⛔ MECHANICAL ENFORCEMENT (v9.0 — 2026-06-02, council 3.66/5 Operational):** The loop is ONLY counted as fired if `Skill("council-skill")` was called as an actual tool use in this response. Mental generation of options without the tool call = loop DID NOT fire. HARD CHECK before writing 🔁 table: scan this response's tool call history for a `Skill` call with input matching `council`. If absent AND firing conditions are met → you MUST call `Skill("council-skill")` NOW before writing the footer. If you cannot (context too high to safely run council) → write 🔁 table with single row `No-op · loop skipped — context too high for safe council · Bennett sees options only` and post to #leo-coaches `C0AQ4KB1SA0` "LOOP_SKIPPED: context=Xk, reason=<>". NEVER write a filled 🔁 table without the tool call receipt. Presenting 5 options to Bennett without calling council-skill = the exact autonomy failure Bennett identified 2026-06-02.

**⛔ EXECUTE-DISPATCH ENFORCEMENT (v9.1 — 2026-06-02):** After `council-skill` returns its JSON verdict, each item rated **EXECUTE** MUST have a corresponding tool call (`Skill`, `Bash`, `MCP`, or `Write`) made in this response before the round advances. Council returning EXECUTE for option X without a tool call implementing X = auto-execute failure. HARD CHECK before printing `ROUND N:` summary: scan this response's tool-call history and count EXECUTE items from the council JSON vs tool calls dispatched for them. If any EXECUTE item has no corresponding tool call → dispatch it NOW. A round is only marked complete when all EXECUTE items have actual tool-call receipts. This is the complement to v9.0: **v9.0 ensures council-skill is CALLED; v9.1 ensures EXECUTE items are DISPATCHED.**
...
- **4.61 Session receipt lock (NEW HARD GATE):** every non-lite response must leave a session-scoped memory receipt in `~/.openclaw/state/memory-skill-receipts.jsonl` using `session_id` or `thread`. No-op scans still write a receipt with `written: []` plus `skipped`. If the receipt is missing, the response is incomplete even if the footer renders.
...
- **7 / 7.5** update Notion Live Thread row (verify HTTP 200; on non-200: **retry 3x with 2s backoff** — silent 429/503 drop was root cause of 'memory never synced' [fleet council fix 2026-06-04]); else `NOTION_UPDATE_FAIL`; Drive-fail fallback → Notion All-Tasks page + #leo-coaches (`C0AQ4KB1SA0`). (#leo-auto C0AKXT2S1T2 is broken — never use it; canonical fallback is #leo-coaches.)
...
- `🧠 MEMORY: 🟢 Green` means shared/fleet memory was actually saved, with canonical proof such as a Drive memory file ID plus readback, a Notion canonical-target write receipt, or a memory-skill receipt tied to that canonical write.
- `🧠 MEMORY: 🟡 Yellow` means Notion, Drive, Gmail, or local receipts may have been updated, but shared/fleet memory itself was not saved yet. Local receipt paths never upgrade this to Green.
...
## v9.3 PATCH — Recap Memory Receipt Hard Gate (2026-06-03)

Every non-lite recap on Codex/ChatGPT must do memory work, not only render a MEMORY status line. Before `final_response_marker_check` is reported, the draft must have:

- memory proof: memory-skill receipt tied to a canonical Drive/Notion write, or canonical Drive memory file ID plus readback; OR
- `memory partial:<exact blocker>` and a reconciliation receipt path. Local receipt paths may support reconciliation, but never count as saved-memory proof.

If the user asks for `permanent fix`, `handoff`, `closeout`, `recap`, or `memory saved`, actual memory proof is required before the final response. A yellow MEMORY line alone is a failure even when the marker checker passes.

## v9.5 PATCH — Durable Claim Proof Gate (2026-06-04)

Permanent/fleet-wide/canonical/shared-memory claims are a separate proof class from normal recap footer proof. Before saying `permanent`, `fleet-wide`, `canonical`, `memory saved`, `Green`, `perfect`, `Diamond`, `done`, or `verified`, the agent must prove the exact changed source with one of:

- Drive canonical write plus fetchback/readback from the same file ID;
- `gog --replace`/Drive REST replace proof plus fetchback SHA or size;
- Notion canonical page/row write receipt plus readback;
- memory-skill receipt tied to a canonical Drive/Notion memory write.

A `recap-skill proof:` line only proves the recap skill was fetched. It does not prove a durable behavior change, memory save, or fleet-wide patch. Local checker files, local receipts, local ad_hoc memory notes, Slack handoffs, and Notion project-state updates can support reconciliation, but must stay `partial`/Yellow unless canonical proof exists.

If a final-answer checker passes while relying on local-only durable proof, the checker is wrong and must be patched before the final response.


## v9.9 PATCH — Number Namespace / Duplicate Bare Number Guard (2026-06-04)

Bennett-facing recap outputs must not show duplicate bare numbered lines. Use `Q1.` style for Questions and Answers. The final next-actions block is the only place bare `1.` through `5.` may appear. This prevents the confusing two `3.` lines while preserving the Drive-canonical recommendation format `3. ⭐`. Checker requirement: fail any final draft that has extra bare option-like `1.`-`5.` lines outside the final five-option block.
