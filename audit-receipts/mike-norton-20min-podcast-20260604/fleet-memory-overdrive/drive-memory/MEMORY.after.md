...
## 2026-06-04 — Blueprint AI NotebookLM origin and sequential gate hardening

Source thread: Chad/Codex, Blueprint AI — Mike Norton A-to-Z Verification, Notion page https://app.notion.com/p/374cf5514fd38116a10af88c504def54
Repo: /Users/temp/fki-preview

### What went wrong
- Mike Norton audio was previously allowed to reach Bennett preview after NotebookLM failed and local/macOS/Gemini-style fallback audio was accepted by size, duration, direct-address, and hash gates.
- The gate stack checked output quality but did not require NotebookLM origin proof as a hard prerequisite.
- A stale transcript file could remain bound to an older MP3 and make run-audit evaluate the wrong audio evidence.
- A prior final response treated local receipts/ad_hoc notes as stronger fleet-memory proof than they were.

### Fleet rule now required
- For any Blueprint AI production podcast, Step 19 Audio render and Step 20 Audio transcript/readback must require true NotebookLM origin proof before Bennett preview, Gatekeeper, Completion, Conveyor, or external send can pass.
- Required proof chain: notebook_id, source_id, artifact_id or generation receipt, downloaded audio receipt, public/local SHA match, transcript/direct-address readback, and listening QA.
- Local TTS, Gemini TTS, macOS say, or any non-NotebookLM fallback is RED/BLOCKED_PENDING_NOTEBOOKLM, not previewable.
- Conveyor 30 must be sequential fail-closed: the first RED step forces later steps RED with blocked_by_prior_step_XX until the blocker is fixed. Step 30 remains LOCKED_HUMAN_GATE until Bennett approval plus external_send token.
- Podcast transcript reuse must be SHA-bound to the current audio via metadata; stale transcript files are ignored and the gate falls back to current-audio transcription or fail-closed.
- Local memory notes and repo receipts are not fleet memory. Fleet learning requires Drive/Notion canonical write plus fetchback/readback.

### Proof from the corrected run
- Drive blueprint-ai-skill canonical file ID: 1IzE-seDDaB1Tciik9mwBVrgQcb7FP7eH, version v3.28, fetchback SHA de3bcdfe0d9a9f8c4614ebcb2ae658a35fa9d6b7062ea18d081532c9ab6847d0.
- Repo final proof commit: 71995b7d pushed to main.
- Mike true NotebookLM artifact: notebook 518433f6-8f0e-43d6-bcfb-bfcdf891dd69, source e373d4f6-eae7-4c1b-b59a-74bf978208d4, artifact 196c4923-4ef9-48d0-8e0f-b59b415aeaa5.
- Mike latest public audio: https://bennett-maxwell.github.io/fki-preview/podcasts/mike-norton-origins-20260603.mp3?v=71995b7d, SHA 4b73dc0712e2c77507915a64b2fa6ca1d7806406f8e687f3052a860e8467835b, duration 1195.46 seconds, size 9564297 bytes.
- Final gates: run-audit PASS 15/15; Completion Gate PASS 44/44; Gatekeeper PASS 100; Conveyor 30 PASS_PREVIEW_ONLY with 29 GREEN, 1 LOCKED_HUMAN_GATE, 0 RED.
- Notion row updated to Awaiting Approval; no customer/Mike send happened.

### How future agents verify they learned this
- Search Drive first for blueprint-ai-skill, load current SKILL.md, and verify v3.28 or newer before touching a Blueprint package.
- Run Conveyor 30 before preview/send and inspect Step 19/20 for NotebookLM origin proof.
- Rerun Completion Gate, Gatekeeper, and run-audit after any audio or transcript change.
- If any step is RED, do not preview, do not send, and do not claim Diamond; update Notion with the exact blocker and proof path.
