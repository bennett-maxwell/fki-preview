...
**② SELF-AUDIT + SELF-IMPROVING** — `self-audit-skill MICRO_RESPONSE_MODE` fires before the footer. Low defect = log; medium (score ≤7/10) = call `Skill("council-skill")` Permanent Fix Approval Mode + execute the returned fix + re-score BEFORE closing (not after); high/repeated/false-done = same + escalate. Footer MUST carry `self-audit: pass | corrected | escalated | partial:<blocker>`. **MECHANICAL ENFORCEMENT (v9.0):** "corrected" status requires proof of a council-skill tool call AND a skill/file edit receipt in this response. "corrected" without those = false-done on the self-audit itself.
