...
patched_v3_28: "2026-06-04 — NotebookLM podcast origin hard gate. A Blueprint production podcast cannot pass Bennett preview, Gatekeeper, Completion, or Conveyor Step 19/20 if audio was created by local TTS, Gemini TTS, macOS say, or any fallback. NotebookLM auth/CLI/MCP partial is RED, not a previewable partial. Required proof: notebook_id, source_id, artifact_id or NotebookLM generation receipt, downloaded audio receipt, public/local SHA binding, transcript/direct-address readback, and listening QA. If NotebookLM fails, stop and label BLOCKED_PENDING_NOTEBOOKLM; do not send Bennett preview. This patch was added after the Mike Norton 20-min podcast incorrectly passed size/hash/direct-address gates with local TTS fallback."
patched_v3_27: "2026-06-04 — Blueprint Scale Conveyor hardening. This is the Madison scale version. The skill is now explicitly versioned for high-throughput Blueprint production: every Blueprint package must run through the 30-step conveyor, Step 23 visual/customer-view email gate, Step 14 Q7 tailoring, and Step 30 external-send human gate. Scale claims such as 1000/day are blocked unless a synthetic scale-smoke fixture passes after the Drive skill update. Required fixture: Billy Bob / Billy Bob Electric, electrician/electrical contractor, avg customer value $1000, annual revenue $2M, no external send. The fixture must produce profile, blueprint HTML, delivery email, qualifier context proof, visual email gate, completion gate, Gatekeeper result, conveyor-30 audit, self-audit receipt, and memory receipt. Any failure is SCALE_PARTIAL, not permanent/green. Sub-agents may own individual conveyor steps, but each step must leave a receipt before the next protected step."
patched_v3_26: "2026-06-02 — Madison troubleshoot: (1) ZERO-BYPASS audit gate — before any Stage 7 email action, confirm blueprint-ai-audit-skill has run and score=100/100 in THIS session. If not: HALT, run audit, block email. No session-memory exception. No 'audit already ran yesterday' bypass. (2) EMAIL TEMPLATE FREEZE — Stage 7 delivery email (build-delivery-email.sh output) is READ-ONLY for structure, subject, and CTA. AI populates token values (lead name, URL, etc.) but NEVER redesigns, rewrites, or restructures the email body. If template modification is needed: propose-only, flag to Bennett, do NOT auto-apply. Reason: Madison session skipped Stage 6 audit and regenerated email template from scratch — both permanently blocked by this patch."
patched_v3_25: "2026-06-02 — Follow-up campaign proof hardening. Gatekeeper 100 is a Blueprint production/content gate only, not proof that GHL apply/qualifier follow-up campaigns are going out. Full E2E/Diamond/goal_complete claims are blocked until ghl-followup-campaign-final-readback PASS proves both categories on the same contact: apply workflow/campaign evidence and qualifier workflow/campaign evidence. Tags/notes/repeat-submit can prove relay/contact identity; failed source=app SMS, zero native campaigns, zero native form submissions, or missing enrollment-history endpoint require BLOCKED/PARTIAL labels and Kay/Jenn workflow proof, not overstatement."
patched_v3_24: "2026-06-02 — Automation trigger route hardening. GHL webhook handlers must call scripts/blueprint-pipeline-orchestrator.py with --profile for the exact submitted lead profile; never call deprecated scripts/blueprint-batch.sh, which is intentionally blocked. Pipeline Stage 6 must use format-conformance + blueprint_completion_gate.py / Gatekeeper 100, not stale pre-delivery CTA counters. Required proof: bash -n handler, py_compile orchestrator, --profile dry-run passes 1/1 with --dry-run no-send, production completion gate 44/44 and Gatekeeper 100 PASS after route repair."
...
patched_v3_29: "2026-06-05 — Customer-view profile-contamination hard gate. Gatekeeper and Step 23/27 cannot pass when the visible Blueprint agent cards, opportunity map, customer profile, or customer-facing copy conflict with the lead profile, even if qualify.html links carry correct lead/biz/src/agents. Required prevention proof: blueprint_qualifier_context_gate.py must use the lead profile as source-of-truth, visible agent-name cards must match the profile agents, and a negative replay of the contaminated Billy Bob SaaS/CRM page must fail with visible_agents_do_not_match_lead_profile. Also gen-blueprint.py must replace ROI_MIN/ROI_STEP so regenerated pages cannot leave template tokens."
description: >
...
## PATCHED 2026-06-05 — v3.29 CUSTOMER-VIEW PROFILE-CONTAMINATION HARD GATE

**Status:** canonical Drive skill patch for Billy Bob / all Blueprint AI agents.

This patch exists because a Billy Bob Electric preview carried correct `qualify.html` link context but the visible Blueprint still showed SaaS/CRM copy and generic agent cards. That must never score green again.

### Non-negotiable rule
A Blueprint package is invalid when the customer-visible page conflicts with the lead profile, even if hidden links, query params, or receipts look correct.

### Required visible-profile proof
Before Step 23, Step 27, Bennett preview, Gatekeeper, Completion, or external send can pass, the proof must show:
1. `leads/<slug>.json` or the canonical lead profile is loaded as the source of truth;
2. visible `.agent-name` cards match the profile-specific agents;
3. `qualify.html` links carry the same agents, `lead`, `biz`, and `src`;
4. visible profile/business-type/results/opportunity sections do not contain cross-lead industry copy;
5. regenerated pages contain no unresolved `{{TOKEN}}` placeholders;
6. a negative replay of a known contaminated page fails the gate.

### Hard-fail conditions
Mark RED / `BLOCKED_PROFILE_CONTAMINATION` if any of these are present:
- visible agents do not match the lead profile;
- the page contains stale industry words from another lead, such as SaaS/CRM copy on an electrician profile;
- hidden qualifier context is correct but visible page content is wrong;
- `gen-blueprint.py` or the active generator leaves `ROI_MIN`, `ROI_STEP`, or any `{{...}}` token unresolved.

### Repo implementation to use
When running in `/Users/temp/fki-preview`, use:
- `scripts/gen-blueprint.py` with ROI token replacement;
- `scripts/blueprint_qualifier_context_gate.py` profile-first matching;
- `scripts/blueprint_gatekeeper_100.py` with the lead profile path;
- a negative replay receipt proving contaminated Billy Bob visible agents fail.

### Claim rule
- `GREEN`: visible page, qualifier links, delivery email, and profile all agree.
- `YELLOW`: preview-ready but Bennett/customer sends are not proven.
- `RED`: any visible/profile mismatch or cross-lead copy contamination.

---



## PATCHED 2026-06-04 — v3.28 NOTEBOOKLM PODCAST ORIGIN HARD GATE

**Status:** canonical Drive skill patch for Madison / all AI agents.

This patch exists because a Mike Norton 20-minute podcast passed duration, size, SHA, and direct-address checks even though NotebookLM failed and a local TTS fallback was used. That must never score green again.

### Non-negotiable rule
A Blueprint production podcast is invalid unless it is generated through the NotebookLM Blueprint podcast skill and leaves origin proof.

### Required NotebookLM origin proof
Before Step 19, Step 20, Bennett preview, Gatekeeper, Completion Gate, or external send can pass, the receipt must include at least one complete NotebookLM proof chain:
1. `notebook_id`,
2. `source_id`,
3. `artifact_id` or NotebookLM generation receipt,
4. downloaded MP3 path,
5. public/local SHA match,
6. transcript/direct-address readback,
7. listening QA receipt.

### Hard-fail conditions
Mark RED / `BLOCKED_PENDING_NOTEBOOKLM` if any of these are present:
- `local_tts_fallback`, `tts_fallback`, macOS `say`, Gemini TTS fallback, or any non-NotebookLM audio generator,
- `notebooklm_status` includes `PARTIAL`, `AUTH`, `EXPIRED`, `FAIL`, `BLOCK`, or `FALLBACK`,
- no NotebookLM artifact/source/notebook/generation receipt,
- size/hash/direct-address pass without NotebookLM origin proof.

### Conveyor enforcement
- Step 19 Audio render = GREEN only when audio size/window AND NotebookLM origin proof pass.
- Step 20 Audio transcript/readback = GREEN only when transcript/direct-address AND NotebookLM origin proof pass.
- Gatekeeper production mode must include `production_audio_notebooklm_origin`.
- Completion Gate production check 47 must fail without NotebookLM origin proof.
- Bennett preview Step 29 cannot run if Step 19 or Step 20 is RED.

### Blocked behavior
If NotebookLM auth or CLI fails, stop. Do not replace it with local TTS and do not call it a successful preview. The correct status is `NotebookLM partial / Blueprint blocked`, not green.

---

## PATCHED 2026-06-04 — v3.27 BLUEPRINT SCALE-SMOKE CONTRACT

**Status:** canonical Drive skill scale patch for Madison / all AI agents.

Before anyone claims Blueprint AI is production-scale, permanently fixed, or ready for high-throughput operation, run a synthetic fixture after the current Drive `SKILL.md` has been fetched in the active session.

### Required synthetic scale fixture
- Lead name: `Billy Bob`.
- Business: `Billy Bob Electric`.
- Industry: electrician / electrical contractor.
- Average customer value: `$1000`.
- Annual revenue: `$2,000,000`.
- External send: forbidden; Step 30 must remain `LOCKED_HUMAN_GATE`.

### Required scale-smoke receipts
A scale-smoke pass requires all receipts below in one run folder:
1. lead profile JSON,
2. Blueprint HTML artifact,
3. delivery email artifact,
4. qualifier context gate,
5. delivery email visual gate,
6. completion gate,
7. Gatekeeper result,
8. Conveyor 30 audit,
9. self-audit/edit receipt,
10. memory-skill receipt tied to Notion/Drive proof.

### Scale claim rule
- `GREEN`: all required receipts pass and Step 30 is locked, not bypassed.
- `SCALE_PARTIAL`: any receipt is missing, stale, generic, or local-only.
- `RED`: any customer/external send occurs without Bennett approval or `external_send` token.

### Sub-agent rule
Use sub-agents only as bounded step workers/auditors. A sub-agent output is not accepted unless it writes a receipt that the parent conveyor verifies.

---
## PATCHED 2026-06-04 — BLUEPRINT CONVEYOR 30 HARD GATE

**Status:** canonical Drive skill patch for Madison / all AI agents.

Blueprint AI must run as a 30-step conveyor. Do not treat Blueprint work as one giant task. Every step must produce a worker output, an auditor/gate result, and a receipt before the next protected stage can proceed.

### Greenlight rule
- Steps 01-28 must be GREEN before Bennett sees an approval preview.
...
- Step 30 is external/customer send and remains `LOCKED_HUMAN_GATE` until Bennett explicitly approves and Gatekeeper token includes `external_send`.
- Final audit must output all 30 steps as `GREEN`, `RED`, or `LOCKED_HUMAN_GATE`; no prose-only pass claims.

### 30 steps
01 Intake identity lock.
02 Source-of-truth bundle.
03 Brand/business classification.
04 Tool stack extraction.
05 GHL/contact readback.
06 Opportunity map.
07 Agent list.
08 Prompt pack.
09 Financial inputs.
10 Blueprint HTML render.
11 Blueprint copy QA.
12 CTA/link QA.
13 Qualifier required fields.
14 Q7 tailoring.
15 Lead relay/GHL payload.
16 Repeat submit/idempotency.
17 Calendar routing.
18 Audio script.
19 Audio render — must include NotebookLM origin proof; local/Gemini/macOS TTS fallback is RED.
20 Audio transcript/readback — must include transcript/direct-address readback plus NotebookLM origin proof.
21 Delivery email data.
22 Delivery email technical HTML.
23 Delivery email visual format.
24 Bennett approval packet.
25 Blueprint audit.
26 Completion gate.
27 Gatekeeper token.
28 Public readback.
29 Bennett preview send.
30 External/customer send.

### Step 23 — Delivery email visual format is a hard gate
This step exists because prior approval previews technically passed but looked wrong. The email visual-format audit must verify:
- customer-view subject and body,
- no internal proof, Codex, Gatekeeper, SHA, Notion, or receipt language,
- `<!DOCTYPE html>` at the top with no metadata comments before it,
- approved card layout with visible border/depth,
- button styling,
- exactly one `qualify.html` CTA,
- `lead`, `biz`, `src`, and `agents` context on the qualifier link,
- rendered screenshot/readback receipt when tooling is available.
...
A valid post-preview package should show `29 GREEN, 1 LOCKED_HUMAN_GATE, 0 RED`. External/customer send is never green until Bennett approval plus `external_send` token.

---


# Blueprint AI Skill v3.27 — Master Orchestrator
...
## v3.25 Follow-Up Campaign Proof Gate (Chad/Codex proof — 2026-06-02)

Use this section whenever the task asks to verify follow-up campaigns for Blueprint AI apply submits and Qualify submits.

1. Treat `blueprint_gatekeeper_100.py --mode production` as a Blueprint production/content gate only. It proves the Blueprint, podcast/player, funnel links, render, and delivery-wrapper requirements; it does not prove GHL follow-up campaigns are going out.
2. Full E2E closeout cannot exceed `PRODUCTION_READY_NOT_SENT_WITH_FOLLOWUP_PARTIAL`, Yellow, or `goal_complete:false` until `audit-receipts/<slug>/ghl-followup-campaign-final-readback-*.json` has `status: PASS` and positive proof for both categories below.
3. Apply follow-up PASS requires same contact ID + exact contact count `1`, apply tags/source, and at least one native follow-up proof: published apply workflow enrollment/readback, native campaign enrollment/readback, or successful same-contact outbound where `source` shows workflow/campaign/automation. A failed reserved-phone SMS with `source=app` is only partial.
4. Qualifier follow-up PASS requires same contact ID + exact contact count `1`, qualifier tags/notes, qualification band, and a published qualifier/Advaita workflow or campaign whose trigger is the qualifier tag/form/source, plus enrollment/readback or successful workflow/campaign-sourced outbound. Generic dialer, not-qualified, draft, or unrelated workflows do not count.
5. Relay-backed custom pages may produce zero native GHL form submissions. Zero native form submissions is not a failure for relay contact identity if tags/notes/repeat-submit prove correctly; it is a failure only when claiming a native form-triggered workflow/campaign fired.
6. If the public API lacks a contact enrollment-history GET endpoint, try read-only fallback in this order and save receipts: HighLevel connector search/fetch, direct API workflow list, contact tags/notes/tasks/appointments, conversations/messages with source/status, forms submissions with corrected `q` filters, campaigns list, and Chrome UI direct workflow/contact route. If all are unavailable or inconclusive, label `BLOCKED_FOLLOWUP_CAMPAIGN_PROOF` or category-level partial.
7. Route unresolved workflow-publish/enrollment proof to Kay/Jenn/GHL admin in #leo-coaches or the active project row, not Bennett, unless a true protected action is present. Do not create/publish workflows, send external messages, or delete/merge contacts without current approval.
8. Future agents must not mark Diamond, `goal_complete:true`, `DELIVERED_TO_PROSPECT`, or customer-ready full E2E when the follow-up proof receipt is blocked/partial, even if the Blueprint artifact itself is 100/100.

## v3.24 Automatic Trigger Route Hardening (Chad/Codex proof — 2026-06-02)

Use this section whenever the task includes proving that a public Blueprint AI apply submit automatically starts Blueprint creation.

1. The GHL webhook handler must route each accepted submit to `scripts/blueprint-pipeline-orchestrator.py --profile "$PROFILE" --workers 1 --batch-size 1` for the exact profile written from the webhook payload.
2. `scripts/blueprint-batch.sh` is deprecated and intentionally exits blocked; any webhook, cron, LaunchAgent, or workflow that still calls it is an automatic-creation blocker.
3. The route must support `BLUEPRINT_WEBHOOK_DRY_RUN=true` for no-send proof and `BLUEPRINT_WEBHOOK_FOREGROUND=true` for smoke tests. A smoke test must not send a Bennett preview or prospect/customer email.
4. Pipeline Stage 6 must use the current completion stack: `format-conformance-check.py` when present, then `blueprint_completion_gate.py --html blueprints/<slug>.html --receipt-dir audit-receipts/<slug> --lead <slug> --json-output`. Do not use stale pre-delivery counters such as old Apply CTA counts as the decision gate.
5. Stage 7 may build/send Bennett preview only after `blueprint_gatekeeper_100.py --mode production` mints or verifies the production token. External customer/prospect send remains blocked without Bennett approval in the current session.
6. Required automation-route proof receipt fields: handler syntax PASS, orchestrator py_compile PASS, handler contains `blueprint-pipeline-orchestrator.py`, handler no longer actively calls `blueprint-batch.sh`, orchestrator supports `--profile`, dry-run log shows `Dry run: True` and `COMPLETE: 1/1 passed`, production completion gate remains 44/44, and Gatekeeper 100 remains PASS.
7. If the HighLevel connector is 401 but direct API works, label `HighLevel connector partial` and store direct-API receipts. If direct API can prove tags/notes/conversation but cannot expose a contact workflow enrollment ID, label only the enrollment-ID subproof partial instead of overstating campaign proof.
...
7. Record follow-up proof separately for apply and qualifier categories: tags, notes, conversation/instant-response, and workflow/campaign IDs if the endpoint is available. If workflow/campaign readback is unavailable, label the affected follow-up category partial/blocked and do not say campaigns are going out; `Gatekeeper 100 PASS` does not override this v3.25 follow-up proof gate.
...
4. **Audio** — player exists, 1x and 1.25x speed controls exist, podcast URL/readback exists, and NotebookLM origin proof exists. Explicit podcast partial is allowed only as a blocked label, never as Bennett-preview/customer-send green.
...
4. Download podcast MP3/MP4 — WALKTHROUGH WINDOW 6-20MB (~6-20 min @128kbps). < 6MB = FAIL (too short). > 20MB = FAIL (too long, a lecture not a walkthrough) — re-generate SHORTER (target 12-18 min). The old "≥29MB" floor was BACKWARDS and forced 30+ min podcasts; it is removed. Local TTS/Gemini/macOS fallback is also FAIL even if this size window passes.
...
**Receipt (Council v3.8 + v3.28):** Write `receipts/stage-4-notebooklm.json` with status, notebook_id, source_id, artifact_id or generation_receipt, podcast_url, file_size_mb, sha256, direct_address_readback, listening_qa, and gates_passed. If NotebookLM auth/CLI/MCP fails, receipt status must be RED/BLOCKED_PENDING_NOTEBOOKLM and the conveyor must stop before Bennett preview.
...
**[v3.26 ZERO-BYPASS GATE — Madison patch]:** Before proceeding to Stage 7, confirm blueprint-ai-audit-skill has run and returned score=100/100 IN THIS SESSION. "Ran yesterday" / "ran last cycle" = NOT valid. If not confirmed: HALT, execute audit now, do not proceed to Stage 7 or any email action until score=100/100 is verified in the current session.
...
**[v3.26 EMAIL TEMPLATE FREEZE — Madison patch]:** Stage 7 delivery email template structure, subject line, and CTA copy are READ-ONLY. `build-delivery-email.sh` populates token values (lead name, URL, podcast link) only. NEVER redesign, rewrite, or restructure the email body. If a template modification is desired: write a proposal to `/tmp/<slug>-email-change-proposal.md` and flag to Bennett. Do NOT auto-apply. Reason: session skipped audit AND regenerated email from scratch — both permanently blocked by v3.26.
...
9. Verify apply and qualifier follow-up campaigns/workflows using the v3.25 Follow-Up Campaign Proof Gate before any full E2E/Diamond/goal_complete claim
